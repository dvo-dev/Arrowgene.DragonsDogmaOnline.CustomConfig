    ...
    public override double GetMultiplier(GameMode gameMode, CharacterCommon characterCommon, PartyGroup party, InstancedEnemy enemy, QuestType questType)
    {
        ...
        switch (gameMode)
        {
            case GameMode.Normal:
                multiplier = 1.2 * LibDdon.GetSetting<double>("GameServerSettings", "EnemyExpModifier");
                break;
            case GameMode.BitterblackMaze:
                multiplier = 1.2 * LibDdon.GetSetting<double>("GameServerSettings", "BBMEnemyExpModifier");
                break;
            default:
                break;
        }
